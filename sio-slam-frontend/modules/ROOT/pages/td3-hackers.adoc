...
. Dans la vue en liste, ajouter les opérations de modification et suppression d'un item (se référer à la  <<_maquette_de_home_component_html>>, mais placer des icones significatifs à la place des mots "modifier" et "supprimer")
 ...
 . Le bouton _submit_ du formulaire est actuellement labellisé "Ajouter / Mettre à jour".
+
Modifier *dynamiquement* (comprendre par du code, pas staitiquement) ce comportement afin d'afficher soit *Ajouter*  soit *Mettre à jour* pour mieux informer l'utilisateur (UX)

. Ajouter le lien vers le drapeau du pays (obtenu par l'API externe) comme nouvelle propriété de la <<_structure_des_données>> d'un Hacker, et présenter le drapeau dans la liste et la vue formulaire.
