...
. Vérifier que l'application ne soumet à l'API distant que des valeurs d'IP saisie par l'utilisateur qui soient au format *IPV4* ou *IPV6* (prévoir un filtre qui utilise la technologie des *expressions régultières*)

. Ne pas soumettre non plus des adresses non routables. Voir : https://www.ibm.com/docs/fr/networkmanager/4.2.0?topic=translation-private-address-ranges[Adresses non routables]
