...
Il semblerait que la variable `{{ipAddress}}` dans la vue réceptionne la valeur par défaut de la variable. Trouvez un moyen pour corriger cela.

Il existe plusieurs façons d'opérer. Faites des recherches en utilisant les moyens que vous avez à disposition, en mode connecté à internet bien entendu.
