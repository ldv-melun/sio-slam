...
IMPORTANT: Vous ne devez jamais retoucher le code de ces librairies de `node_modules` !
...
. Faire en sorte que l'application ne présente que le message `Hello World` lorsqu'elle est lancée, à l'image de la capture écran ci-dessous.
...
. Faire en sorte que la page affiche l'IP du client qui l'a sollicité.
====
